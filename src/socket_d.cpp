...
void tcp_socket_d::open(const std::vector<connection_info> & infos, tcp_flags::flags f) {
	close();
	flags = f;
	for (const connection_info & info : infos) {
		handle = socket((int)info.endpoint.addr_family(), info.sock_type |
			((f & tcp_flags::non_blocking) ? SOCK_NONBLOCK : 0), info.protocol);
		if (handle == -1)
			continue;
...
int open_listener(const std::string & address, std::uint16_t port, endpoint_info & local_info,
		int sock_type, tcp_flags::flags flags) {
	addrinfo initial, *sysaddr = nullptr, *a = nullptr;
	std::memset(&initial, 0, sizeof(addrinfo));
	initial.ai_family = AF_UNSPEC;
	...
	for (a = sysaddr; a != nullptr; a = a->ai_next) {
		handle = socket(a->ai_family, a->ai_socktype |
			((flags & tcp_flags::non_blocking) ? SOCK_NONBLOCK : 0), a->ai_protocol);
		if (flags & tcp_flags::reuse_port) {
			int opt = 1;
...
tcp_listener_d::tcp_listener_d(tcp_flags::flags f) : flags(f), local_info(endpoint_info::empty) {
	handle = -1;
}

tcp_listener_d::tcp_listener_d(const std::string & address, std::uint16_t port,
		tcp_flags::flags f) {
	handle = -1;
	listen(address, port, f);
}

void tcp_listener_d::listen(const std::string & address, std::uint16_t port, tcp_flags::flags f) {
	close();
	flags = f;
...
udp_server_d::udp_server_d(const std::string & address, std::uint16_t port, tcp_flags::flags f) {
