...
struct cli_error : public std::runtime_error {
	explicit cli_error(const std::string & message) : std::runtime_error(message) {}
	explicit cli_error(const char * message) : std::runtime_error(message) {}
};
...
struct cli_choice_error : public cli_error {
	explicit cli_choice_error(const std::string & message) : cli_error(message) {}
};
...
struct word_cli_form : public cli_form {
	...
	explicit word_cli_form(const std::string & init) : word(init) {}
...
class word_cli_node final : public chain_cli_node {
	std::string name;
public:
	explicit word_cli_node(const std::string & form) : name(form) {}
	virtual void process(const char * str, std::unordered_map<std::string, cli_form_ptr> & forms) override;
	virtual std::vector<std::string> help() override;
};

struct cli_form_error : public cli_error {
	explicit cli_form_error(const std::string & message) : cli_error(message) {}
	explicit cli_form_error(const char * message) : cli_error(message) {}
};
